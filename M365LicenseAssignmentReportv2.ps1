...
try {
    ...
    if ($connected) {
        ...
        $users = Invoke-GraphRequestWithPaging -Uri "beta/users?`$select=UserPrincipalName,LicenseAssignmentStates,DisplayName,AccountEnabled,AssignedLicenses,signInActivity,country,usageLocation,countryLetterCode&`$top=999"
        ...
        foreach ($user in $users) {
            ...
            $country = if ($user.Country) { $user.Country }
               elseif ($user.usageLocation) { $user.usageLocation }
               elseif ($user.countryLetterCode) { $user.countryLetterCode }
               else { "Unknown" }
...
# Last Login Activity
$country = if ($user.Country) { $user.Country } else { "Unknown" }
